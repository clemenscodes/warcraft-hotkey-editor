...
pub(crate) enum SystemHotkeysCategory {
    Inventory,
    HeroSelection,
    ControlGroups,
    GeneralCommands,
...
impl SystemHotkeysCategory {
    pub(crate) const ALL: [SystemHotkeysCategory; 8] = [
    ...
    /// Section IDs that belong to this category, in display order.
    pub(crate) fn entries(self) -> Vec<&'static SystemKeybind> {
        match self {
