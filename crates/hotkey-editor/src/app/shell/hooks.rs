...
use crate::app::shell::route_sync::NavDecision;
use crate::app::shell::style;
...
/// Everything the [`Shell`](super::Shell) body needs: the root class, the app-level
/// key handler, and the signals the overlay dialogs are rendered from. Every other
/// piece of state the shell owns is handed to the page tree and the header through
/// context, so the body is a flat list of children — no god-bag of props.
