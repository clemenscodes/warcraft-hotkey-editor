...
pub(super) fn GridTile(props: GridTileProps) -> Element {
    ...
    let cell_with_slot =
        Positions::cell_for_position(&slot_ids, custom_keys_option, is_research_grid, column, row);
    let raw_occupant_slot: Option<GridSlotId> =
        cell_with_slot.as_ref().map(|occupant| occupant.slot_id());

    let morph_reverse_cell: Option<AbilityCell> = raw_occupant_slot.as_ref().and_then(|slot| {
        let GridSlotId::Ability(ability_id) = slot else {
            return None;
        };
        let ability_id_str = ability_id.value();
        let binding = custom_keys_option.and_then(|file| file.binding(ability_id_str));
        if let Some(target) = ObjectLookup::morph_target_unit(ability_id_str)
            && target.eq_ignore_ascii_case(host_unit_id)
        {
            return Some(AbilityCell::for_ability_off(*ability_id, binding));
        }
        if !is_uprooted_grid
    ...
    let is_selected = match (&occupant_slot, active_slot.as_ref()) {
        (Some(occupant), Some(active)) => {
            occupant == active && active_selection_is_research == is_research_grid
        }
        _ => false,
    };
    ...
    let drag_in_progress_from_this_section: bool = {
        let guard = dragging_slot.read();
        guard
            .as_ref()
            .is_some_and(|detail| detail.source_section() == heading_text)
    };
    let dragging_id_str: Option<String> = {
        let guard = dragging_slot.read();
        guard
            .as_ref()
            .filter(|detail| detail.source_section() == heading_text)
            .map(|detail| detail.slot_id().as_str().to_string())
    };
    ...
    let is_drop_target_cell = drag_in_progress_from_this_section
        && drop_target_cell
            ...
            .as_ref()
            .map(|target| {
                target.section() == heading_text && target.column() == column && target.row() == row
            })
            .unwrap_or(false);
    ...
    let is_off_state_blocked = !is_research_grid
        ...
        && cell_option.is_none()
        && slot_ids.iter().any(|slot| {
            let GridSlotId::Ability(ability_id) = slot else {
                return false;
            };
            if dragging_id_str
                .as_deref()
                .is_some_and(|id| ability_id.value().eq_ignore_ascii_case(id))
            {
                return false;
            }
    ...
    let cell_tier_index = cell_object_id_option
        ...
        .unwrap_or(0);
    let cell_database_object = cell_object_id_option.and_then(|id| ObjectLookup::by_id(id.value()));
    let cell_tier_name = cell_database_object
        .and_then(|warcraft_object| warcraft_object.names().get(cell_tier_index).copied())
        .map(String::from);
    let cell_tier_icon = cell_database_object
        .and_then(|warcraft_object| warcraft_object.icons().get(cell_tier_index).copied())
    ...
    let displayed_letter: Option<String> =
        binding_letter_option.or_else(|| derived_letter.map(|character| character.to_string()));
