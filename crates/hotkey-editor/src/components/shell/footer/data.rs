use super::components::footer_credit::FooterCreditProps;
use super::components::footer_disclaimer::FooterDisclaimerProps;
use super::components::footer_link::FooterLinkProps;
...
/// The authorship line, sourced here and threaded into `FooterCredit` as props.
pub const CREDIT: FooterCreditProps = FooterCreditProps {
    lead: "Crafted with",
    tail: "by Clemens",
    heart: HEART_SVG,
};

/// The outbound links, looped over by the footer. The GitHub entry carries an
/// inline glyph; the others are plain text links (`icon` is `None`).
pub const LINKS: &[FooterLinkProps] = &[
    FooterLinkProps {
...
pub const DISCLAIMER: FooterDisclaimerProps = FooterDisclaimerProps {
    text: "Not affiliated with or endorsed by Blizzard Entertainment. \
