...
/// The burger templates row's shaped data: the fixed icon and label, its idle weight, its aria
/// state, whether its browser is open, the toggle handler, and the change handler the mounted
...
/// Owns the burger's own templates-browser open signal (closed until the row is tapped) and
/// shapes the row: the toggle handler that opens or closes the browser, and the change handler
