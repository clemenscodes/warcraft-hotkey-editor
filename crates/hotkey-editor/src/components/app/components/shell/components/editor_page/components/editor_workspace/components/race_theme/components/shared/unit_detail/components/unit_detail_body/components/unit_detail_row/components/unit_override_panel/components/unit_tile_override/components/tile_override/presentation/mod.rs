use super::model::{
    OverrideEditing, PositionPickers, TileOverrideInputs, TileOverrideModel,
    TileOverridePresentation,
};
...
pub(super) fn use_tile_override(props: &TileOverrideModel) -> TileOverridePresentation {
    let detail = props.detail.clone();
    let loaded_keys = use_loaded_keys();
    let grid_layout = use_grid_layout();
    let editor = use_editor_state();
    ...
    TileOverridePresentation::from(inputs)
}
