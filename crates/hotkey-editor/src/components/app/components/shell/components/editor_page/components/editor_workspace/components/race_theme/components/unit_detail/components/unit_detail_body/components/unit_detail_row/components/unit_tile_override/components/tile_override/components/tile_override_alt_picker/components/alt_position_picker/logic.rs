...
/// The off-state picker's own shell, shaped from its model: the open value driving
/// the backdrop, the change handler that writes the open signal, the header props,
/// and the scroll-region body props. Every dialog owns its shell now — there is no
/// base.
