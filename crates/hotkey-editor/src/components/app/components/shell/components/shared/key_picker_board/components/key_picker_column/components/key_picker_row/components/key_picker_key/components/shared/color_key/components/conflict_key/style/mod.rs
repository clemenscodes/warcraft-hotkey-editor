use tw_macro::tw;

classes! {
    base: tw![
        "relative",
        "group/tooltip",
        "[anchor-name:--tooltip-anchor]",
        "[anchor-scope:--tooltip-anchor]",
        "min-w-0",
        "w-full",
        "h-[7cqi]",
        "px-1",
        "flex",
        "items-center",
        "justify-center",
        "border",
        "rounded-control",
        "text-[1.7cqi]",
        "leading-none",
        "cursor-pointer",
        "whitespace-nowrap",
        "transition-[border-color,background,box-shadow]",
        "duration-fast",
        "text-shadow-outline",
        "focus:outline-none",
        "kb-focus:outline-none",
        "kb-focus:border-white",
        "kb-focus:text-white",
        "kb-focus:[--focus-color:var(--color-warcraft-highlight)]",
        "kb-focus:shadow-focus",
        "disabled:cursor-not-allowed",
        "disabled:opacity-85",
        "[background:color-mix(in_oklab,var(--color-warcraft-danger)_50%,transparent)]",
        "border-warcraft-danger",
        "text-warcraft-danger",
        "[&:hover:not(:disabled)]:border-warcraft-danger",
        "[&:hover:not(:disabled)]:[background:color-mix(in_oklab,var(--color-warcraft-danger)_55%,transparent)]",
        "[--glow-color:var(--color-warcraft-danger)]",
        "[&:hover:not(:disabled)]:shadow-glow-soft",
    ],
    mobile: tw![
        "mobile:h-[8.5cqi]",
        "mobile:p-0",
        "mobile:text-[1.6cqi]",
    ],
}
