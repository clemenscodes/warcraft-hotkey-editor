use crate::components::app::components::shell::components::editor_page::components::editor_workspace::components::race_theme::components::shared::unit_detail::components::unit_detail_body::components::unit_detail_row::components::shared::grid_editors::grid_editor::GridEditorProps;
...
impl<B: GridBehavior> From<&GridEditorProps<B>> for DragFollowerOverlayProps {
    /// The follower overlay needs only the editor's drag signal and whether the
    /// in-progress drag started in this grid. The last is true when the dragging slot
    /// reports this grid's id. Its ghost reads the race accent from `--race-accent`.
