...
use presentation::use_hotkey_alt_position_picker_dialog_host;
use style::CLASS;
use tw_macro::assert_component;

/// Mounts the off-state position picker only while it is open, reading its own open signal
/// to decide — the way `CarriersDialogHost` reads its open state. It renders the reusable
...
pub fn HotkeyAltPositionPickerDialogHost(props: HotkeyAltPositionPickerDialogHostModel) -> Element {
    let dialog = use_hotkey_alt_position_picker_dialog_host(&props);
