...
/// The Orc race tab: publishes Orc's banner colour, accent, strength, and banner image
/// as the `--race-*`/`--banner-*` custom properties the shared chrome reads, then
...
/// `style.rs`.
#[component]
pub fn OrcRaceTab(props: RaceTabsProps) -> Element {
