...
use tw_macro::tw;
// The shared gold key-cap surface for the two editable hotkey cells (the editor's
// override key and the layout-grid tile). It fills the box its host button hands it
// (`size-full`) and draws the whole cap — border, gold fill, glyph colour, outline
// shadow, hover glow, and the capture pulse (`states!`). Font *size* is not set here:
// it inherits from the host, so each host picks its own scale (`text-2xl` vs
// `text-5xl`) while the cap look stays shared. Focus is a pseudo of the *host* button
// (the focusable, keyboard-navigable element), so the keycap reflects it through the
// host's `group/editable-keycap` marker rather than its own `:focus-visible`. The
// corner radius is the one look variant, driven by `data-radius`.
