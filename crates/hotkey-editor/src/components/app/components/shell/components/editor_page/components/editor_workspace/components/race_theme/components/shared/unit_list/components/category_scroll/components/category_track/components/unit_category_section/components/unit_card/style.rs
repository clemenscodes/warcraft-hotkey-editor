...
// in `cqh`) and the per-kind carousel visibility filter, which reads the
// `data-unit-kind` attribute this wrapper renders. All of the card's look — border,
// fill, hover, focus, and the `--race-color` selected accent — lives on the surface.
