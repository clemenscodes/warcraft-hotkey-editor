...
pub(super) const GAIN: ClassList = ClassList::new(
    "flex-[0_0_auto] ml-auto text-right text-race-human text-xl font-normal [font-variant-numeric:tabular-nums] text-shadow-drop data-[zero=true]:text-warcraft-text-faint data-[zero=true]:font-normal mobile:text-xl",
);
