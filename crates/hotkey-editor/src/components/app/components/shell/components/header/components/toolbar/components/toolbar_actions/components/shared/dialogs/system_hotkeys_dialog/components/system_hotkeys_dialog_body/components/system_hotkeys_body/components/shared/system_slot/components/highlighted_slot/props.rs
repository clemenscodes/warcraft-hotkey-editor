...
/// The highlighted-look slot's props: the already-shaped caption, bound-key, and
/// tooltip child props, plus the two orthogonal flags — `compact` (the tighter
/// control-group cell) and `dragging` (this slot is being dragged, so its contents
/// hide). Built by the dispatcher from `SystemSlotProps`; carrying the child props as
