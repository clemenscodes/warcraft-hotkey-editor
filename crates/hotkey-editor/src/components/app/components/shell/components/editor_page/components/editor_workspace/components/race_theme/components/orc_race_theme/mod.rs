...
assert_component!(OrcRaceTheme);

/// The Orc race-themed container. A `display:contents` wrapper that publishes the
/// Orc colour as `--race-color` and `--race-accent` for its descendants — the unit
