...
assert_component!(UndeadRaceTab);

/// The Undead race tab: publishes Undead's banner colour, accent, strength, and banner
/// image as the `--race-*`/`--banner-*` custom properties the shared chrome reads, then
/// forwards the navigation to the shared state dispatcher for its race. A thin per-race
/// modifier — its only difference from the other four tabs is the four values in its
/// `style.rs`.
#[component]
pub fn UndeadRaceTab(props: RaceTabsProps) -> Element {
