...
impl GridLayout {
        ...
        for row_index in 0..histograms.len() {
            for column_index in 0..histograms[row_index].len() {
                ...
                let chosen = most_common
                    .or_else(|| fallback.letter_at(column_u8, row_u8))
                    .unwrap_or(' ');
    ...
    pub(crate) fn letter_at(&self, column: u8, row: u8) -> Option<char> {
    ...
    pub(crate) fn assign_unique(&mut self, column: u8, row: u8, letter: char) {
